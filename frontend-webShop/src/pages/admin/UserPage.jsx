  ...
  return (
    <div className="flex flex-col w-full bg-background min-h-screen">
      <section className="w-full max-w-1600px mx-auto pt-10 px-12">
        <UniversalHeader title={viewMode === "BANNED" ? "Blacklist" : "Identity_Logs"} subtitle="DIRECTORY_V3.0" isAdmin={viewMode === "ADMIN"} onActionClick={() => setIsModalOpen(true)} searchTerm={searchTerm} setSearchTerm={setSearchTerm} />
      ...
      <section className="w-full max-w-1600px mx-auto px-12 pb-24">
        <UniversalTable data={mappedData} columns={["Entity_Name", "Identity_Mail", "Access_Level", "Status", "Execute"]} isAdmin={true} onDelete={(id) => { setUserToDelete(users.find(u => u.id === id)); setIsDeleteModalOpen(true); }} onEdit={(id) => { setSelectedUserId(id); setIsModalOpen(true); }} />
      </section>
